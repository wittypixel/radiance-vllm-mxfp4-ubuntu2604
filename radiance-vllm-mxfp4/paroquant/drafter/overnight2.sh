...
D=$HOME/drafter_ft; S=$HOME/deadcode-vllm/paroquant/drafter; LOG=$D/overnight2.log
NEW=Qwen3.8-27B-DFlash2-FP8-paro2; OLD=Qwen3.8-27B-DFlash2-FP8
say() { echo "$(date +%H:%M:%S) $*" | tee -a "$LOG"; }
# 1. wait for the round-2 generation client (exits when done)
until [ -f $D/gen3.done ]; do sleep 60; done   # the generation chain touches this when the client exits
say "generation finished: $(wc -l < $D/responses2.jsonl) responses, $(ls $D/capture2 | wc -l) capture files, $(du -sh $D/capture2 | cut -f1)"
...
run_train() {
  podman run --rm --privileged --ipc=host --device /dev/kfd --device /dev/dri --group-add keep-groups --security-opt seccomp=unconfined \
    ...
    "cd /data && python3 /scripts/train_drafter.py --capture /data/capture,/data/capture2 --drafter /data/ft_bf16 --target /models/Qwen3.8-27B-embed-head --out /data/ft2_bf16 --epochs 1.5 --lr 6e-5 --val 96 --eval-every 150 --save-every 300 $*" >> $D/train2.log 2>&1
}
say "training: seqs 6 x anchors 64"; run_train --seqs 6 --anchors 64 && ok=1 || ok=0
...
grep -E "\[eval" $D/train2.log | tail -20 | tee -a "$LOG"
...
podman run --rm -v $D:/data:z -v $HOME/models:/models:z -v $S:/scripts:z --entrypoint bash stilldeadcode/vllm-radiance:0.9.3 -lc \
  "python3 /scripts/export_fp8.py /data/ft2_bf16 /models/$OLD /models/$NEW" 2>&1 | tee -a "$LOG"
...
$S/gate_drafter.sh $NEW new2 2>&1 | tee -a "$LOG"; podman stop -t 30 vllmparomx >/dev/null 2>&1; sleep 8
# old drafter gated 2026-09-09 (gate/old.*): reuse
cn=$(grep -oE "median ≈ \*\*[0-9.]+" $D/gate/new2.bb.log | grep -oE "[0-9.]+$"); co=$(grep -oE "median ≈ \*\*[0-9.]+" $D/gate/old.bb.log | grep -oE "[0-9.]+$")
gn=$(grep -oE "accuracy [0-9.]+" $D/gate/new2.gsm8k.log | grep -oE "[0-9.]+"); go=$(grep -oE "accuracy [0-9.]+" $D/gate/old.gsm8k.log | grep -oE "[0-9.]+")
