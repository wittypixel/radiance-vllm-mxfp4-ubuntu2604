...
class DFlash2Qwen3DecoderLayer(DFlashQwen3DecoderLayer):
    def __init__(
        ...
        *,
        ...
        # The block size comes from the SERVE, not from the checkpoint, and nothing upstream checks
        # the two against each other. A block-diffusion drafter denoises a fixed number of mask
        # positions and was trained for one block length: measured on this checkpoint at c1 greedy,
        # acceptance is 4.205 at the trained 8 and 3.961 at 9 -- a silent 7% of throughput for a
        # serve flag that looks like it should only make the proposal longer.
