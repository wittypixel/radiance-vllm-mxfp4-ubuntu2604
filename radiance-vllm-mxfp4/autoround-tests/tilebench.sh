...
hipcc -O3 -w -std=c++17 --offload-arch=gfx1201 -Rpass-analysis=kernel-resource-usage \
  tilebench.hip -o /tmp/tilebench > /work/tilebench_build.log 2>&1 || { echo BUILD FAILED; tail -40 /work/tilebench_build.log; exit 1; }
echo '--- prefill variant resources (VGPR / occ / LDS / spill) ---'
python3 - <<'PY'
import re
cur=None
for ln in open('/work/tilebench_build.log'):
